import {cityLandscape} from './landscape.js';
import {environment} from '../textures.js';
import {studyArchitecture} from './study-architecture.js';
import {activityParcels,empireInstance,ambientTraffic} from './visual-layer.js';
...
export default function Skyline3D({
  ...
  cameraPreset="midtown", libertyAsset=null, study = true, activity = false, empireAsset = null, traffic = false,
  ...
  useEffect(() => {
    ...
    const cameraKey=`${activity}|${cameraPreset}|${focus}|${focusNonce}`;
    ...
    if(study && !activity)Object.assign(TOD,time==='night'?{land:0x3c4b48,road:0x283632,pave:0x65716a,back:0x394638,park:0x354a32,water:0x284852,amb:.22,hemi:.7,sun:0x9bbbd8,sunI:1,ground:'#7b6d59',top:'#648398',horizon:'#263944'}:{land:0x8d8d81,road:0x555c59,pave:0xbeb7a4,back:0x86907c,park:0x7f936c,water:0x628b90,amb:.38,hemi:.95,sun:0xffe4b6,sunI:2.6,ground:'#7b6d59',top:'#c9e2ee',horizon:'#b8c7c8'});
    ...
    let { env, sky } = skyEnv(renderer, TOD);
    let studyEnv=null;
    if(study && !activity){env.dispose();sky.dispose();sky=environment(time);const pm=new THREE.PMREMGenerator(renderer);studyEnv=pm.fromEquirectangular(sky);pm.dispose();env=studyEnv.texture;renderer.toneMappingExposure=time==='night'?.8:1.04;}
    ...
    let cinematicDelivered=false;
    const draw = () => { if (composer) composer.render(); else renderer.render(scene, cam); labelR.render(scene, cam); if(onCinematicFrame&&!cinematicDelivered){cinematicDelivered=true;onCinematicFrame(renderer.domElement.toDataURL('image/png'));} };
    ...
    if (city) {
      ...
      if(activity){water.material=new THREE.MeshBasicMaterial({color:0x060e18});chop.visible=false;waterTick=null;}
      ...
      for (const w of WATER) flat([w.ring], activity?0x060e18:TOD.water, -0.16, true);
      ...
      for (const r of park.water) flat([r], activity?0x060e18:TOD.water, 0.10, true);      // reservoir/lake/pond as real water
    ...
    if(study && !activity) cityLandscape(scene,K,libertyAsset,disposables,ownMats);
    ...
    const architecture=study && !activity?studyArchitecture(scene,time,disposables,ownMats):null;
    T.forEach((t, i) => {
      ...
      let widest = 0;
      widest = Math.max(...parts.map(q=>Math.max(q.w,q.d)));
      if (!activity && empireAsset && i===0) scene.add(empireInstance(empireAsset,{x:p.x,z:p.z,height:roofTop+spire,width:widest,material:bucket.mat,disposables}));
      if(architecture && !(empireAsset && i===0)) architecture.emit(t,i,h+spire,p.x,p.z,widest);
      if (!activity && !study && !(empireAsset && i===0)) {
      ...
      } // Visual replacement only; rank, placement and signals remain unchanged.
      ...
      picks.push({ t, x: p.x, z: p.z, w: widest * 1.02, h: activity ? .25 : h + spire });
      if (city) { t.hood = placed[i].hood; homes.set((t.a || "").toLowerCase(), { x: p.x, z: p.z, h: h + spire }); }
      if (i === 0) champInfo = { x: p.x, z: p.z, h: h + spire };
    });

    architecture?.finish();
    if(activity) activityParcels(scene,T,pts,maxFlow,disposables,ownMats);
    ...
    if (city) {
      ...
      for (const a of agg.values()) {
        ...
        Object.assign(d.style, {
          color: "rgba(226,232,240,0.72)", font: study && !activity ? "500 9px system-ui" : "600 12px 'Space Grotesk', system-ui, sans-serif",
    ...
    const frameCbs = [];
    if(traffic && !activity) frameCbs.push(ambientTraffic(scene,K,disposables,ownMats));
    if(!activity && study && !focus && cameraPreset!=='overview'){
      const candidate=cameraPreset==='midtown'?picks[0]:picks.find(q=>q.t.hood==='village' && q.h<6)||picks[Math.floor(picks.length*.5)];
      const d=cameraPreset==='midtown'?45:15;
      cam.position.set(candidate.x+d*.8,cameraPreset==='midtown'?30:11,candidate.z+d);controls.target.set(candidate.x,cameraPreset==='midtown'?8:2,candidate.z);controls.update();
    }
    if(!activity && study && !focus && ['park','harbor'].includes(cameraPreset)){
      let x,z,d;
      if(cameraPreset==='park'){const ring=PARK_RINGS[0];x=ring.reduce((sum,p)=>sum+p[0],0)/ring.length*K;z=ring.reduce((sum,p)=>sum+p[1],0)/ring.length*K;d=32;}
      else {const mon=siteAt(SITES.monument,K),b=bridgeSpan(K);x=(mon.x+(b.ax+b.bx)/2)/2;z=(mon.z+(b.az+b.bz)/2)/2;d=65;}
      cam.position.set(x+d*.6,d*.8,z+d*.75);controls.target.set(x,0,z);controls.update();
    }
    if(!activity && focus){const home=homes.get(focus.toLowerCase());if(home){cam.position.set(home.x+home.h*.85,home.h*.85,home.z+home.h*.85);controls.target.set(home.x,home.h*.48,home.z);controls.update();}}
    if(activity){
      // Source map uses +z north. Reflect the presentation to give a north-up map.
      scene.scale.z=-1;scene.fog=null;
      for(const g of groundBits){if(g.material?.emissive)g.material.emissiveIntensity=0;}
      const xs=pts.map(p=>p.x),zs=pts.map(p=>-p.z);
      const minX=Math.min(...xs),maxX=Math.max(...xs),minZ=Math.min(...zs),maxZ=Math.max(...zs);
      const height=Math.max((maxZ-minZ)*1.15,(maxX-minX)*1.25/(W/VH))/(2*Math.tan(THREE.MathUtils.degToRad(23)));
      const cx=(minX+maxX)/2-(maxX-minX)*.13,cz=(minZ+maxZ)/2;
      cam.position.set(cx,height,cz+.001);controls.target.set(cx,0,cz);controls.enableRotate=false;controls.maxPolarAngle=.001;controls.update();
    }
    if(cameraMemory.current?.key===cameraKey){cam.position.copy(cameraMemory.current.position);controls.target.copy(cameraMemory.current.target);controls.update();}
    ...
    const onWheel = e => {
      ...
      if(activity){_off.setLength(THREE.MathUtils.clamp(_off.length()*Math.exp(e.deltaY*.001),controls.minDistance,controls.maxDistance));cam.position.copy(controls.target).add(_off);controls.update();return;}
    ...
    return () => {
      cameraMemory.current={key:cameraKey,position:cam.position.clone(),target:controls.target.clone()};
      ...
      disposables.forEach(d => d.dispose()); env.dispose(); sky.dispose();studyEnv?.dispose();
  ...
  }, [towers, isMobile, crownLabel, accent, bodyFrom, bodyTo, layout, intro, time, infra, arcs, beamAll, activity, empireAsset, traffic, focusNonce, study, libertyAsset, cameraPreset]);
