...
// `cacheable(url)` decides what may be read back from disk. A one-off sweep caches everything (its
// rerun must see the same pages). A REFRESH must not: an address's token-transfer pages grow over
// time, so a cached one would hand back an old ledger that no longer reconciles. Transaction pages
// never change once mined, so IMMUTABLE_ONLY keeps those and re-reads everything else.
export const IMMUTABLE_ONLY = u => /\/transactions\/0x[0-9a-f]{64}/i.test(u);
export function cachedFetch(dir, { tries = 10, cacheable = () => true } = {}) {
  ...
  const f = async (u, opts) => {
    ...
    for (let i = 0; i < tries; i++) {
      try {
        ...
        if (r.ok) { const body = await r.text(); JSON.parse(body); if (cacheable(u)) writeFileSync(p, body); misses++; return { ok: true, status: 200, json: async () => JSON.parse(body) }; }
